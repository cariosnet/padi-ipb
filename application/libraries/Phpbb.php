...
class Phpbb
{
    ...
    function load_session_id()
    {
    	...
    	$phpbb_config = './forum/config.php';       // Get config path from options
    	$phpbb_root_path = './forum/';            // Get phpBB root path
    	...
    	exit;require($phpbb_root_path . 'includes/constants.' . $phpEx);                        // Load constants file
    	require($phpbb_root_path . 'includes/db/' . $dbms . '.' . $phpEx);                 // Load database file
    
    	$user = new user();                            // Apply filters on $user object
    	$auth = new auth();                                   // Apply filters on $auth object
    	$template = new template(); 
    	$cache = new cache();    
    	$db = new $sql_db();                                             // Apply filters on $cache object
    
    	// Connect to MySQL database
    	$db->sql_connect($dbhost, $dbuser, $dbpasswd, $dbname, $dbport, false, defined('PHPBB_DB_NEW_LINK') ? PHPBB_DB_NEW_LINK : false);
    	unset($dbpasswd);                                                       // Unset the password
    
    	$config = $cache->obtain_config();                   // Apply filters on $config object
                               
    	$user->session_begin();                                             // Run actions if any is attached before we assign permissions
    	$auth->acl($user->data);                                 // Run actions if any is attached
    	$user->setup();      
