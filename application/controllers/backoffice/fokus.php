...
class Fokus extends CI_Controller {
	...
	public function index(){
		...
		$data = array(
				'pageTitle' 	=> 'Fokus',
				'content'	 	=> 'back/fokus/fokus-list',
				'contentData'	=> array(
						'listNews'		=> $this->X_News_Model->getListNewsNoLimit("F", "A")
				)
		);
	...
	public function create(){
		$data = array(
				'pageTitle' 	=> 'Buat Fokus Baru',
				'content'	 	=> 'back/fokus/fokus-add',
				'contentData'	=> array(
		...
		if($this->input->post('save') != ''){
			...
			if($_FILES['news_picture']['name'] != ''){
				...
				if (!$this->upload->do_upload('news_picture')){
					...
					if($id != null){
						redirect('backoffice/fokus/edit/'.$id);
					}else{
						redirect('backoffice/fokus/create');
					}
			...
			if($id != ''){
				...
				if($_FILES['news_picture']['name'] != ''){
					redirect('backoffice/fokus/croping/'.$id);
				}
			}else{
				...
				if($_FILES['news_picture']['name'] != ''){
					redirect('backoffice/fokus/croping/'.$lastInsertedId);
				}
			}
				
			$this->session->set_flashdata('success', 'Data berhasil disimpan.');
			redirect('backoffice/fokus');
		}
		redirect('backoffice/fokus/create');
	}
	...
	public function croping($id){
		...
		$data = array(
				...
				'content'	 	=> 'back/fokus/fokus-croping',
				'contentData'	=> array(
						'row'		=> $obj->row()
				)
		);
	...
	function edit($id){
		...
		$data = array(
				'pageTitle' 	=> 'Ubah fokus',
				'content'	 	=> 'back/fokus/fokus-edit',
				'contentData'	=> array(
						'news'			=> $obj->row(),
	...
	function delete($id){
		...
		if($obj->num_rows() != 0){
			...
			if($this->X_News_Model->delNews($obj->row()->NEWS_ID)){
				...
				$this->session->set_flashdata('success', 'Fokus "'.$obj->row()->NEWS_TITLE.'" berhasil dihapus.');
			}else{
...
/* End of file fokus.php */
/* Location: ./application/controllers/backoffice/fokus.php */